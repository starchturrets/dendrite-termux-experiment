...
The [official Dendrite documentation](https://matrix-org.github.io/dendrite/installation/database) provide instructions for how to set it up. The Termux version does have some slight differences in syntax, however. 
      
      $ pkg install postgresql
      $ 
      $ createuser -P dendrite
      $ postgres createdb -O dendrite dendrite
      $ for i in appservice federationapi mediaapi mscs roomserver syncapi keyserver userapi; do
          createdb -O dendrite dendrite_$i
        done

Setup a termux-service to have PostgreSQL start up automatically by running `sv-enable postgres`.
